...
class Model(object):
    def __init__(self, item_count, genre_count, model_config):
        ...
        self.ip_len = tf.placeholder(tf.float32, [None, 3, None])  # 每部电影拥有的情节的个数
        ...
        self.hist_ip_len = tf.placeholder(tf.float32, [None, None, 3, None])  # [B, T, 3, item_embedding]
        ...
        self.plot_emb_w = tf.get_variable("plot_emb_w",
                                     [genre_count, self.plot_embedding_size],
                                     initializer=initializer,
                                     dtype=tf.float32)
